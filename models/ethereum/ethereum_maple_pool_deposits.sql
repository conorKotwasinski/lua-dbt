with pool_transactions as (
    ...
    where to_address in (select address from seeds.maple_pools)
        and substr(input, 1, 10) = '0x2e1a7d4d' -- withdraw function selector
),
...
select
    ...
    transaction_hash,
    event,
    lower(event[1].2)      as transfer__from,
    lower(event[2].2)      as transfer__to,
    toUInt256(event[3].2)  as transfer__value,
    lower(event[4].2)      as points_correction_updated__account,
    event[5].2   as points_correction_updated__points_correction,
    lower(event[6].2)      as losses_correction_updated__account,
    event[7].2   as losses_correction_updated__losses_correction,
    lower(event[8].2)      as funds_withdrawn__by,
    event[9].2   as funds_withdrawn__funds_withdrawn,
    event[10].2  as funds_withdrawn__total_withdrawn,
    event[11].2 as balance_updated1__balance,
    lower(event[12].2)     as balance_updated1__liquidity_provider,
    lower(event[13].2)     as balance_updated1__token,
    lower(event[14].2)     as losses_recognized__by,
    event[15].2 as losses_recognized__losses_recognized,
    event[16].2 as losses_recognized__total_losses_recognized,
    event[17].2 as balance_updated2__balance,
    lower(event[18].2)     as balance_updated2__liquidity_provider,
    lower(event[19].2)     as balance_updated2__token
from pool_events;
...
select
    *
from ethereum_test.demo_maple_pool_deposits as mpd
    left join seeds.maple_pools as
    left join ethereum.blocks as b on mpd.block_number = b.number;
