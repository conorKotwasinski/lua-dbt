...
run_dbt () {
    if [[ "$1" == "test" ]]
    then
        dbt deps && dbt seed --profiles-dir=. --target prod && dbt build --profiles-dir=. --target prod
    else
        dbt deps && dbt seed --profiles-dir=. --target prod && dbt run --profiles-dir=. --target prod
    fi
}
